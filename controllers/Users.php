<?php
    ...
    class Users {
        ...
        public function register(){
            ...
            if(!preg_match("/^[a-zA-Z0-9]*$/", $data['usersUid'])){
                flash("register", "Неверное имя пользователя");
                redirect("../signup.php");
            }

            if(!filter_var($data['usersEmail'], FILTER_VALIDATE_EMAIL)){
                flash("register", "Неверный адрес электронной почты");
                redirect("../signup.php");
            }
            ...
            if(strlen($data['usersPwd']) < 6){
                ...
                flash("register", "Пароли не совпадают");
                redirect("../signup.php");
            }

            //User with the same email or password already exists
            if($this->userModel->findUserByEmailOrUsername($data['usersEmail'], $data['usersName'])){
                flash("register", "Имя пользователя или электронная почта уже заняты");
                redirect("../signup.php");
            }
    ...
    public function login(){
        ...
        if(empty($data['name/email']) || empty($data['usersPwd'])){
            flash("login", "Пожалуйста, заполните все поля");
        ...
        if($this->userModel->findUserByEmailOrUsername($data['name/email'], $data['name/email'])){
            ...
            if($loggedInUser){
                ...
                flash("login", "Неверный пароль");
                redirect("../login.php");
            }
        }else{
            flash("login", "Пользователь не найден");
            redirect("../login.php");
        }
    }
